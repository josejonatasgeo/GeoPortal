...
define(['underscore',
            ...
            if (files.hasOwnProperty(name)) {
                ...
                if (layers.hasOwnProperty(name)) {
                    ...
                    if (Object.keys(layers).length == 0) {
                        ...
                        log_error({
                            title: 'Seleção Errada',
                            message: gettext('Apenas um arquivo de cada vez pode ser carregado!')
                        });
                    }
                }
            }
        ...
        $.each(layers, function (name, info) {
            if (!info.type) {
                log_error({
                    ...
                    message: interpolate(gettext('O arquivo %s é um tipo de arquivo não suportado, selecione outro arquivo.'),[info.files[0].name])
                });
                delete layers[name];
            } else {
    ...
    doUploads = function () {
        if ($.isEmptyObject(layers)) {
            common.logError('Por favor, forneça alguns arquivos');
            return false;
        }

        var checked = checkFiles() && checkGeogig();
        if ($.isEmptyObject(layers) || !checked) {
            alert(gettext('Você está tentando enviar um conjunto incompleto de arquivos ou nem todas as opções obrigatórias foram validadas. \n \nVerifique se há erros no formulário!'));
        } else {
