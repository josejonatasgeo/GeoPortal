...
from .forms import LayerAttributeForm
...
@login_required
def attribute_upload(request,id):
    attribue = get_object_or_404(Attribute,pk=id)
    form = LayerAttributeForm(request.POST or None,instance=attribute)

    if form.is_valid():
        form.save()
        return HttpResponseRedirect('layer_detail')
    return render(request,'layer_detail.html',{'form':form})
...
def layer_metadata(
        request,
        layername,
        template=['layers/layer_metadata.html','layers/layer_detail.html'],
        ajax=True):
    layer = _resolve_layer(
        request,
        layername,
    ...
    if request.method == "POST":
        ...
        tkeywords_list = ''
        lang = 'pt-br'  # TODO: use user's language
