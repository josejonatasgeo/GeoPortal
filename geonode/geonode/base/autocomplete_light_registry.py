...
register(Region,
         search_fields=['name'],
         autocomplete_js_attributes={'placeholder': 'Região/Cidade ..', },)

register(ResourceBaseAutocomplete,
         search_fields=['title'],
         order_by=['title'],
         limit_choices=100,
         autocomplete_js_attributes={'placeholder': 'Nome recurso..', },)

register(HierarchicalKeyword,
         search_fields=['name', 'slug'],
         autocomplete_js_attributes={'placeholder':
                                     'Um espaço ou uma lista separada por vírgulas de palavras-chave', },)
...
if hasattr(settings, 'THESAURI'):
    for thesaurus in settings.THESAURI:
        ...
        logger.debug('Registrando o enciclopédia autocomplete para {}: {}'.format(tname, ac_name))
