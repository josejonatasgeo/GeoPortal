...
class CategoryChoiceField(forms.ModelChoiceField):
    ...
    def label_from_instance(self, obj):
        return '<i class="fa '+ obj.fa_class + ' fa-2x unchecked"></i>' \
               '<i style="background-color:black" class="fa ' + obj.fa_class + ' fa-2x checked"></i>' \
               ...
               '<br/><strong style="color:rgba(6, 146, 103, 0.85);font-size:15px">' + obj.gn_description + '</strong></span>'
...
class ResourceBaseForm(TranslationModelForm):
    ...
    temporal_extent_start = forms.DateTimeField(
        label=_("inicio da extensao temporal"),
        required=False,
        localize=True,
        input_formats=['%Y-%m-%d %H:%M %p'],
        widget=ResourceBaseDateTimePicker(options={"format": "YYYY-MM-DD HH:mm a"})
    ...
    temporal_extent_end = forms.DateTimeField(
        label=_("fim da extesao temporal"),
        required=False,
        localize=True,
        input_formats=['%Y-%m-%d %H:%M %p'],
        widget=ResourceBaseDateTimePicker(options={"format": "YYYY-MM-DD HH:mm a"})
